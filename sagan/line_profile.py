...
ls_km = con.c.value/1000

__all__ = ['Line_Gaussian', 'Line_GaussHermite', 'Line_template', 'TierContinuum',
...
class Line_MultiGauss(Fittable1DModel):
    ...
    **kwargs : dict
    ...
    def gen_template(self, v, normalized=True):
        '''
        ...
        '''
        flux =  self.amp_c * np.exp(-0.5 * ((v- self.dv_c) / self.sigma_c)**2)
        ...
        for i in range(n_add):
            ...
            flux += amp_w * self.amp_c * np.exp(-0.5 * ((v - self.dv_c - dv_w) / sigma_w)**2)
...
class TierContinuum(Fittable1DModel):
    amplitude = Parameter(default=1.0, bounds=(0.7, 1.3))

    def __init__(self, y_func, x_static=None, **kwargs):
        """
        ...
        if x_static is not None:
            self._cached_y = y_func(x_static)
        else:
            self._cached_y = None
        self._cached_x = x_static
        self.y_func = y_func
    ...
    def evaluate(self, x, amplitude):
        ...
        if self._cached_x is not None and np.array_equal(x, self._cached_x):
            return amplitude * self._cached_y
        else:
            self._cached_x = x
            self._cached_y = self.y_func(x)
            return amplitude * self._cached_y
    ...
    **kwargs : dict
    ...
    def gen_template(self, v, normalized=True):
        ...
        flux : array like
            The flux of the line profile.
        '''
        flux =  self.amp_c0 * np.exp(-0.5 * ((v- self.dv_c) / self.sigma_c)**2)
        ...
        for i in range(n_add):
            ...
            dv_w = getattr(self, f'dv_w{i}')
            sigma_w = getattr(self, f'sigma_w{i}')
            flux += amp_w * self.amp_c0 * np.exp(-0.5 * ((v - self.dv_c - dv_w) / sigma_w)**2)
